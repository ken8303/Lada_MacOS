...
class MosaicDetector:
    def __init__(self, model: Yolo11SegmentationModel, video_metadata: VideoMetadata, frame_detection_queue: PipelineQueue, mosaic_clip_queue: PipelineQueue, error_handler: Callable[[ErrorMarker], None], max_clip_length=30, clip_size=256, device: torch.device | None = None, pad_mode='reflect', batch_size=4, diagnostic_callback=None):
        ...
        try:
            self.diagnostic_window_seconds = max(10.0, float(os.environ.get("LADA_DIAGNOSTIC_WINDOW_SECONDS", "60")))
        except ValueError:
            self.diagnostic_window_seconds = 60.0
        try:
            self.diagnostic_clip_interval = max(1, int(os.environ.get("LADA_DIAGNOSTIC_CLIP_INTERVAL", "10")))
        except ValueError:
            self.diagnostic_clip_interval = 10
        try:
            self.max_detections_per_frame = max(0, int(os.environ.get("LADA_MAX_DETECTIONS_PER_FRAME", "0")))
        except ValueError:
            self.max_detections_per_frame = 0
        try:
            self.min_detection_confidence = max(0.0, min(1.0, float(os.environ.get("LADA_MIN_DETECTION_CONFIDENCE", "0"))))
        except ValueError:
            self.min_detection_confidence = 0.0
        self.dense_filter_dropped_detections = 0
    ...
    def _emit_diagnostic(self, stage, phase=None, **payload):
        ...
        try:
            ...
            logger.debug("MosaicDetector: diagnostic callback failed", exc_info=True)
    ...
    def _create_clips_for_completed_scenes(self, scenes, frame_num, eof) -> StopMarker | None:
        ...
        for completed_scene in sorted(completed_scenes, key=lambda s: s.frame_start):
            ...
            if clip.id % self.diagnostic_clip_interval == 0:
                self._emit_diagnostic(
                    "clip-create",
                    "complete",
                    clipID=clip.id,
                    frameStart=clip.frame_start,
                    frameEnd=clip.frame_end,
                    clipLength=len(clip),
                )
    ...
    def _detection_confidence(self, results: UltralyticsResults, index: int) -> float:
        try:
            confidence = results.boxes[index].conf
    ...
    def _selected_detection_indices(self, results: UltralyticsResults, frame_num: int) -> list[int]:
        indexed_confidences = [
            (index, self._detection_confidence(results, index))
            for index in range(len(results.boxes))
        ]
        if self.min_detection_confidence > 0:
            indexed_confidences = [
                (index, confidence)
                for index, confidence in indexed_confidences
                if confidence >= self.min_detection_confidence
            ]
        indexed_confidences.sort(key=lambda pair: pair[1], reverse=True)
        if self.max_detections_per_frame > 0:
            kept = indexed_confidences[:self.max_detections_per_frame]
        else:
            kept = indexed_confidences
        dropped = len(results.boxes) - len(kept)
        if dropped > 0:
            self.dense_filter_dropped_detections += dropped
            self._emit_diagnostic(
                "dense-filter",
                "progress",
                frameStart=frame_num,
                detectionsInLastBatch=len(kept),
                detectionsInWindow=self.dense_filter_dropped_detections,
                originalDetections=len(results.boxes),
                droppedDetections=dropped,
                maxDetectionsPerFrame=self.max_detections_per_frame,
                minDetectionConfidence=self.min_detection_confidence,
                note=(
                    f"kept={len(kept)} dropped={dropped} "
                    f"minConfidence={self.min_detection_confidence:.2f} "
                    f"maxPerFrame={self.max_detections_per_frame or 'unlimited'}"
                ),
            )
    ...
    def _create_or_append_scenes_based_on_prediction_result(self, results: UltralyticsResults, scenes: list[Scene], frame_num):
        for i in self._selected_detection_indices(results, frame_num):
        ...
        with video_utils.VideoReader(self.video_meta_data.video_file) as video_reader:
            ...
            while not (eof or self.stop_requested):
                ...
                if len(frames) > 0:
                    ...
                    if window_seconds >= self.diagnostic_window_seconds:
                        self._emit_diagnostic(
                            "frame-feed-window",
                            "progress",
                            framesProcessed=frame_num + len(frames) - self.start_frame,
    ...
    def _frame_inference_worker(self):
        ...
        while not (eof or self.stop_requested):
            ...
            with mps_serialized(self.device):
            ...
            window_frames += len(frames)
            window_inference_seconds += inference_seconds
            detection_count = sum(len(result.boxes) for result in batch_prediction_results)
            window_detections += detection_count
            window_seconds = time.monotonic() - window_started_at
            if window_seconds >= self.diagnostic_window_seconds:
                self._emit_diagnostic(
                    "detect-window",
                    ...
                    windowSeconds=window_seconds,
                    framesPerSecond=window_frames / window_seconds if window_seconds > 0 else None,
                    inferenceSeconds=window_inference_seconds,
                    inferencePercent=window_inference_seconds / window_seconds if window_seconds > 0 else None,
                    detectionsInLastBatch=detection_count,
                    detectionsInWindow=window_detections,
                )
                window_started_at = time.monotonic()
                window_frames = 0
                window_inference_seconds = 0.0
