...
class FrameRestorer:
    def __init__(self, device, video_file, max_clip_length, mosaic_restoration_model_name,
        ...
        self.restored_clip_count = 0
        try:
            self.diagnostic_clip_interval = max(1, int(os.environ.get("LADA_DIAGNOSTIC_CLIP_INTERVAL", "10")))
        except ValueError:
            self.diagnostic_clip_interval = 10
        try:
            self.diagnostic_slow_clip_seconds = max(0.0, float(os.environ.get("LADA_DIAGNOSTIC_SLOW_CLIP_SECONDS", "15")))
        except ValueError:
            self.diagnostic_slow_clip_seconds = 15.0
        try:
        ...
        try:
            self.mps_empty_cache_interval = max(1, int(os.environ.get("LADA_MPS_EMPTY_CACHE_INTERVAL", "1")))
        except ValueError:
            self.mps_empty_cache_interval = 1
    ...
    def _emit_diagnostic(self, stage, phase=None, **payload):
        ...
        try:
            ...
            logger.debug("FrameRestorer: diagnostic callback failed", exc_info=True)
    ...
    def _restore_clip(self, clip: Clip):
        ...
        if clip.id % self.diagnostic_clip_interval == 0 or duration >= self.diagnostic_slow_clip_seconds:
            self._emit_diagnostic(
                ...
                framesPerSecond=clip_length / duration if duration > 0 else None,
            )
    ...
    def _collect_garbage(self, clip_buffer):
        ...
        if has_processed_clips:
            if self.device.type == 'cuda':
                ...
                and self.restored_clip_count % self.mps_empty_cache_interval == 0
            ):
                with mps_serialized(self.device):
    ...
    def _clip_restoration_worker(self):
        ...
        window_clips = 0
        window_clip_frames = 0
        window_restore_seconds = 0.0
        while not (eof or self.stop_requested):
            ...
            if clip is EOF_MARKER:
                ...
                restore_seconds = time.monotonic() - restore_started_at
                window_clips += 1
                window_clip_frames += clip_length
                window_restore_seconds += restore_seconds
                # Release MPS driver cached memory to prevent unbounded growth
                self.restored_clip_count += 1
                if (
                    self.device.type == 'mps'
                ...
                if window_seconds >= self.diagnostic_window_seconds:
                    self._emit_diagnostic(
                        "clip-restore-window",
                        "progress",
                        clipsRestored=window_clips,
                        clipFramesRestored=window_clip_frames,
                        windowSeconds=window_seconds,
                        clipFramesPerSecond=window_clip_frames / window_seconds if window_seconds > 0 else None,
                        clipsPerMinute=window_clips * 60 / window_seconds if window_seconds > 0 else None,
                        restoreSeconds=window_restore_seconds,
                        restorePercent=window_restore_seconds / window_seconds if window_seconds > 0 else None,
                        averageClipSeconds=window_restore_seconds / window_clips if window_clips > 0 else None,
                    )
                    window_started_at = time.monotonic()
                    window_clips = 0
                    window_clip_frames = 0
