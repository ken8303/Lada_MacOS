...
def get_video_meta_data(path: str) -> VideoMetadata:
    if shutil.which("ffprobe") is None:
        return _get_video_meta_data_with_pyav(path)
...
def _get_video_meta_data_with_pyav(path: str) -> VideoMetadata:
    """Read the metadata needed by Lada when ffprobe is not installed.

    PyAV already ships the matching libavformat libraries, so this fallback
    keeps the native macOS build self-contained during development.
    """
    with av.open(path, metadata_errors='ignore') as container:
        stream = container.streams.video[0]
        average_rate = stream.average_rate or stream.base_rate or stream.guessed_rate
        base_rate = stream.base_rate or stream.guessed_rate or average_rate
        if average_rate is None or base_rate is None:
            raise RuntimeError("Unable to determine video frame rate")

        fps_exact = Fraction(base_rate)
        ...
        frame_count = int(stream.frames or round(duration * average_fps))
        codec_name = (
            stream.codec_context.codec.name
            if stream.codec_context.codec is not None
            else stream.codec_context.name
        )
