...
class Progressbar:
    def __init__(self, video_metadata: VideoMetadata, progress_callback=None, disable=False):
        self.frame_processing_durations_buffer = []
        self.video_metadata = video_metadata
        self.progress_callback = progress_callback
        self.frames_processed = 0
        self.progress_callback_min_interval = self._get_progress_callback_float_env("LADA_PROGRESS_CALLBACK_INTERVAL", 1.0)
        self.progress_callback_min_delta = self._get_progress_callback_float_env("LADA_PROGRESS_CALLBACK_MIN_DELTA", 0.001)
        self.progress_callback_last_emit = None
        self.progress_callback_last_progress = 0.0
        ...
        self.tqdm = tqdm(dynamic_ncols=True, total=video_metadata.frames_count, bar_format=BAR_FORMAT_TQDM, desc=initial_estimating_bar_suffix, disable=disable)
    ...
    def close(self, ensure_completed_bar=False):
        if ensure_completed_bar:
            ...
            if not self.error and self.tqdm.total != self.tqdm.n:
                ...
                self.tqdm.refresh()
        self.tqdm.close()
        if ensure_completed_bar and self.progress_callback:
    ...
    def update(self):
        ...
        if self.progress_callback:
            progress = min(1.0, self.frames_processed / max(1, self.video_metadata.frames_count))
            if self._should_emit_progress_callback(progress, duration_end):
                remaining = (
                    max(0, self.video_metadata.frames_count - self.frames_processed) * self._get_mean_processing_duration()
                    if self.frame_processing_durations_buffer
                    else None
                )
                self.progress_callback_last_emit = duration_end
                self.progress_callback_last_progress = progress
                self.progress_callback(
                    progress,
                    remaining,
                    frames_processed=self.frames_processed,
                    total_frames=self.video_metadata.frames_count,
                    mean_frame_seconds=self._get_mean_processing_duration(),
                )

    def _get_progress_callback_float_env(self, name: str, default: float) -> float:
        try:
            return max(0.0, float(os.environ.get(name, str(default))))
        except (TypeError, ValueError):
            return default

    def _should_emit_progress_callback(self, progress: float, now: float) -> bool:
        if self.progress_callback_last_emit is None:
            return True
        if progress >= 1.0:
            return True
        if progress - self.progress_callback_last_progress >= self.progress_callback_min_delta:
            return True
        return now - self.progress_callback_last_emit >= self.progress_callback_min_interval
