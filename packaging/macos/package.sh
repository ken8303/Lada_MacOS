...
write_release_notes() {
  ...
  printf '%s\n' "$sha  Lada-Apple-Silicon.zip" > "$OUTPUT_DIR/SHA256.txt"
  {
    ...
    printf '%s\n' "- Stability: serializes Apple MPS model execution after real parallel-MPS crash evidence, defaults to Long Video 45/75/90 clip lengths by profile, retries smaller MPS clips if MPSGraph aborts, and avoids silent CPU fallback unless explicitly enabled"
